...
class DefaultBrowserIntroTests: XCTestCase {
    ...
    func testExistingUser() throws {
        ...
        // Existing users see the callout depending on app launch count only.
        let appLaunchesToShowPopup = [1, 2, 4, 8, 20]
        
        for i in 1...50 {
            let shouldShow = appLaunchesToShowPopup.contains(i)
            
            XCTAssert(DefaultBrowserIntroManager
                        .prepareAndShowIfNeeded(isNewUser: false, launchDate: now) == shouldShow)
        }
        
        // Full new user cycle has completed
        XCTAssert(IntroPrefs.completed.value)
        
        // Date checks do not matter for existing users
        let laterDate = Date(timeInterval: 10.days, since: now)
        XCTAssertFalse(DefaultBrowserIntroManager.prepareAndShowIfNeeded(isNewUser: true,
                                                                         launchDate: laterDate))
    }
