...
public extension WKWebView {
    ...
    func evaluateSafeJavaScript(functionName: String, args: [Any] = [], sandboxed: Bool = true, escapeArgs: Bool = true, asFunction: Bool = true, completion: ((Any?, Error?) -> Void)? = nil) {
        ...
        if asFunction {
            let ret = generateJavascriptFunctionString(functionName: functionName, args: args, escapeArgs: escapeArgs)
            if ret.error != nil {
                if let completionHandler = completion {
                    completionHandler(nil, ret.1)
                }
                return
            }
            javascript = ret.javascript
        }
