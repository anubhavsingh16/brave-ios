        ...
        if url.isBookmarklet {
            ...
            if visitType == .bookmark {
                if let webView = tab.webView, let code = url.bookmarkletCodeComponent {
                    webView.evaluateSafeJavaScript(functionName: code, sandboxed: false, asFunction: false, completion: { _, error in
                        if let error = error {
                            log.error(error)
                        }
                    })
...
extension BrowserViewController {
    ...
    @objc func addCustomSearchEngineForFocusedElement() {
        ...
        webView.evaluateSafeJavaScript(functionName: "__firefox__.searchQueryForField", sandboxed: false, completion: { (result, _) in
