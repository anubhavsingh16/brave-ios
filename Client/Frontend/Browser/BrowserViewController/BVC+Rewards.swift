...
extension Tab {
    func reportPageLoad(to rewards: BraveRewards) {
        ...
        webView.evaluateSafeJavaScript(functionName: "document.documentElement.outerHTML.toString", completion: { html, _ in
        ...
        if shouldClassifyLoadsForAds {
            group.enter()
            webView.evaluateSafeJavaScript(functionName: "document.body.innerText", asFunction: false, completion: { text, _ in
